...
void bta_av_build_src_cfg (UINT8 *p_pref_cfg, UINT8 *p_src_cap)
{
    ...
    if (src_cap.ch_mode & A2D_SBC_IE_CH_MD_DUAL 
      && P_SBC != property_get_int32(A2DP_PREFERRED_ENCODER_PROP, 0)) {
        ...
        APPL_TRACE_EVENT("codec SBC DUAL/Auto mode");
    }
        ...
        if (property_set(A2DP_ACTIVE_ENCODER_PROP, (char*)"1") < 0)
            BTIF_TRACE_ERROR("Failed to set active codec property");
        pref_cap.ch_mode = A2D_SBC_IE_CH_MD_DUAL;
        APPL_TRACE_EVENT("codec SBC DUAL pref mode");
    }
    ...
    if (status == A2D_SUCCESS)
    {
        ...
        if (codec_cfg_supported)
        {
            ...
            case BTA_AV_CODEC_M24:
            {
                ...
                APPL_TRACE_EVENT("%s p_aac_cie->bitrate = %x",__func__, p_aac_cie.bit_rate);
 ...
 */
                ...
                case BTIF_AV_CODEC_SBC:
                    ...
                    if (bta_av_co_audio_codec_match(p_peer->snks[index].codec_caps, BTIF_AV_CODEC_SBC))
                    {
...
#endif
                        {
                            ...
                            APPL_TRACE_EVENT("%s peer codec SBC matched", __func__);
    ...
    if ((bt_split_a2dp_enabled && btif_av_is_codec_offload_supported(AAC)) 
      && P_AAC == property_get_int32(A2DP_PREFERRED_ENCODER_PROP, 0)) {
                        ...
                        if (bta_av_co_audio_codec_match(p_peer->snks[index].codec_caps, BTIF_AV_CODEC_M24))
                        {
                           ...
                           if (bta_av_co_audio_sink_has_scmst(&p_peer->snks[index]))
#endif
                            {
                                ...
                                APPL_TRACE_EVENT("%s peer codec AAC matched", __func__);
        ...
        APPL_TRACE_EVENT("%s AAC is bypassed", __func__);
#endif
                    ...
                    if (codecId ==  A2D_APTX_HD_CODEC_ID_BLUETOOTH && vendorId == A2D_APTX_HD_VENDOR_ID  )
                    {
                        ...
                        if (bta_av_co_audio_codec_match(p_peer->snks[index].codec_caps, A2D_NON_A2DP_MEDIA_CT))
                        {
    ...
    #endif
                            {
                                ...
                                APPL_TRACE_EVENT("%s peer codec aptX HD matched", __func__);
                ...
                if (codecId ==  A2D_APTX_CODEC_ID_BLUETOOTH && vendorId == A2D_APTX_VENDOR_ID)
                {
                    ...
                    if (bta_av_co_audio_codec_match(p_peer->snks[index].codec_caps, A2D_NON_A2DP_MEDIA_CT))
                    {
...
#endif
                        {
                            ...
                            APPL_TRACE_EVENT("%s peer codec aptX matched", __func__);
    ...
    if (bt_split_a2dp_enabled && btif_av_is_codec_offload_supported(AAC)) {
        ...
        APPL_TRACE_EVENT("%s AAC is disabled", __func__);
#endif
    ...
    *p_status = BTIF_ERROR_SRV_AV_FEEDING_NOT_SUPPORTED;
    ...
    case BTIF_AV_CODEC_PCM:
        ...
        if (P_SBC == property_get_int32(A2DP_ACTIVE_ENCODER_PROP, 0)) {
            ...
            if (property_set(A2DP_ACTIVE_ENCODER_PROP, (char*)"2") < 0)
                BTIF_TRACE_ERROR("Failed to set active codec property");
            APPL_TRACE_EVENT("codec SBC DUAL mode");
...
UINT8 bta_av_select_codec(UINT8 hdl)
{
    ...
    else
    {
        tBTA_AV_CO_PEER *p_peer;
        UINT8 index;
        APPL_TRACE_EVENT("%s hdl = %d",__func__,hdl);
        /* Retrieve the peer info */
